use crate::tcp::{AsyncWriteOwnExt, ReadExt, WriteExt};
...
impl C2s {
    pub fn read(state: State, mut reader: impl io::Read) -> io::Result<Self> {
        let packet_id = reader.read_var_int()?;

        match state {
            State::Shake => Self::read_shake_state(packet_id, reader),
            State::Status => Self::read_status_state(packet_id, reader),
            State::Login => Self::read_login_state(packet_id, reader),
            State::Play => Self::read_play_state(packet_id, reader),
        }
        .inspect(|packet| {
            log::debug!(
    ...
    fn read_shake_state(packet_id: VarInt, mut reader: impl io::Read) -> io::Result<Self> {
        match packet_id {
            0x00 => Ok(Self::Handshake {
                protocol: reader.read_var_int()?,
    ...
    fn read_status_state(packet_id: VarInt, mut reader: impl io::Read) -> io::Result<Self> {
        match packet_id {
    ...
    fn read_login_state(packet_id: VarInt, mut reader: impl io::Read) -> io::Result<Self> {
        match packet_id {
            0x00 => {
                let (has_uuid, uuid) = reader.read_uuid()?;
...
impl S2c {
    pub async fn write_to(self, mut writer: impl AsyncWriteOwnExt) -> io::Result<()> {
        match self {
            Self::StatusResponse {
                text,
                version,
...
fn status_to_json(text: String, version: Version, players: Players) -> String {
